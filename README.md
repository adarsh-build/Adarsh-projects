Welcome to my LeetCode Solutions repository! 🚀
This repository contains my solutions to various LeetCode problems that I am solving as part of my journey to improve my Data Structures & Algorithms (DSA) and problem-solving skills. I will be regularly adding solutions, mainly in C++, along with different approaches, explanations, and optimizations whenever possible. The goal is not just to solve problems, but to understand the logic behind them, analyze their time and space complexity, and continuously improve my problem-solving approach. This repository will also serve as a record of my learning and progress throughout my DSA journey.
## 📊 Progress

I will keep updating this repository as I solve more problems.

- 🟢 Easy
- 🟡 Medium
- 🔴 Hard
- 💻 Language: C++
- 🎯 Focus: DSA & Problem Solving

<!---LeetCode Topics Start-->
# LeetCode Topics
## Array
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
| [3903-smallest-stable-index-i](https://github.com/adarsh-build/Adarsh-projects/tree/master/3903-smallest-stable-index-i) |
## Hash Table
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
## Math
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
## Number Theory
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
## Prime Factorization
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
## Euclidean Algorithm
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
## Primality Test
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
## Greatest Common Divisor
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
## Sieve Theory
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
## Prime Number Sieve
|  |
| ------- |
| [2521-distinct-prime-factors-of-product-of-array](https://github.com/adarsh-build/Adarsh-projects/tree/master/2521-distinct-prime-factors-of-product-of-array) |
## Prefix Sum
|  |
| ------- |
| [3903-smallest-stable-index-i](https://github.com/adarsh-build/Adarsh-projects/tree/master/3903-smallest-stable-index-i) |
## String
|  |
| ------- |
| [1309-decrypt-string-from-alphabet-to-integer-mapping](https://github.com/adarsh-build/Adarsh-projects/tree/master/1309-decrypt-string-from-alphabet-to-integer-mapping) |
<!---LeetCode Topics End-->